...
#EDIT HERE
  ...
  provisioner "local-exec" {
    command = "sed s/USER/${var.ssh_user}/ openstack/ansible_bastion_template.txt | sed s/BASTION_ADDRESS/${var.bastion_fips[0]}/ > openstack/no-floating.yml"
  }
...
#EDIT HERE
  ...
  provisioner "local-exec" {
    command = "sed s/USER/${var.ssh_user}/ openstack/ansible_bastion_template.txt | sed s/BASTION_ADDRESS/${element( concat(var.bastion_fips, var.k8s_node_fips), 0)}/ > openstack/no-floating.yml"
  }
